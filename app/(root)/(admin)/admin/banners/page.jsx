...
const BannersPage = () => {
    ...
    const {
        ...
        queryFn: async ({ pageParam }) => await fetchBanners(pageParam),
        initialPageParam: 0,
    ...
    useEffect(() => {
        if (selectAll) {
            const ids = data?.pages?.flatMap(page => page.banners?.map(banner => banner._id)) || [];
                ...
                <CardContent className="pb-5">
                    ...
                    {status === 'pending' ?
                        ...
                        status === 'error' ?
                            ...
                            <>
                                {data?.pages?.flatMap(page => page.banners)?.length === 0 && <div>No banners found.</div>}
